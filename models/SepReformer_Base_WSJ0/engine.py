...
class Engine(object):
    ...
    def _test(self, dataloader, wav_dir=None):
        ...
        with torch.inference_mode():
            csv_file_name_sisnr = os.path.join(self.work_dir, 'test_SISNRi_value.csv')
            csv_file_name_sdr = os.path.join(self.work_dir, 'test_SDRi_value.csv')
            with open(csv_file_name_sisnr, 'w', newline='') as csvfile_sisnr, open(csv_file_name_sdr, 'w', newline='') as csvfile_sdr:
                ...
                for input_sizes, mixture, src, key in dataloader:
                    ...
                    if self.engine_mode == "test_save":
                        if wav_dir == None: wav_dir = os.path.join(self.work_dir, "wav_out")
                        ...
                        sf.write(os.path.join(wav_dir,key[0][:-4]+str(idx)+'_mixture.wav'), 0.5*mixture/max(abs(mixture)), sampling_rate)
                        for i in range(self.config['model']['num_spks']):
                            src = torch.squeeze(estim_src[i]).cpu().data.numpy()
                            sf.write(os.path.join(wav_dir,key[0][:-4]+str(idx)+'_out_'+str(i)+'.wav'), 0.5*src/max(abs(src)), sampling_rate)
                    idx += 1
    ...
    def run(self):
        with torch.cuda.device(self.device):
            ...
            if "test" in self.engine_mode:
                ...
                valid_loss_best = init_loss_time if self.start_epoch > 1 else float("inf")
