...
def get_dataloaders(args, dataset_config, loader_config):
    ...
    for partition in partitions:
        dataset = VnSpeechMixDataset(dataset_config, partition)
...
class VnSpeechMixDataset(Dataset):
    """Read VN-SpeechMix from an extracted directory or directly from its ZIP file."""
    ...
    def __init__(self, config, partition):
        ...
        self.archive_root = PurePosixPath(config.get("archive_root", "VN-SpeechMix/wav16k/min"))
        self._zip = None
        ...
        if all((extracted_partition / name).is_dir() for name in required_dirs):
            ...
            raise FileNotFoundError(
                "VN-SpeechMix 16 kHz was not found. Expected either "
                f"'{self.extracted_root}' or '{self.archive_path}'."
            )

        if not self.examples:
            raise RuntimeError(f"No WAV examples found for VN-SpeechMix partition '{partition}'")
        logger.info(
            f"VN-SpeechMix {partition}: {len(self.examples)} mixtures from {self.storage} storage"
        )
