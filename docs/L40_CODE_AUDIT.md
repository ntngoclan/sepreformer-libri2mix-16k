# Audit code và triển khai NVIDIA L40
...
Code WSJ0 và thư mục `SourceCode` lịch sử không nằm trên đường chạy L40 này và
...
- Requirements L40: không trùng package và không cài đè torch CUDA wheel.
- Dataset archive:
...
## Kiểm tra bắt buộc trên chính L40

Máy audit hiện tại không có môi trường PyTorch/CUDA của máy train, do đó không
thể trung thực tuyên bố CUDA runtime đã đạt. Trên L40 phải chạy:
...
2. báo cáo nhận đúng một L40 và CUDA 12.1 runtime;
...
`run_logs/preflight_l40_report.json`, sửa nguyên nhân trước khi bắt đầu 200 epoch.

## Giới hạn có chủ ý

- Chưa bật AMP để tránh thay đổi điều kiện số học ngay trước thí nghiệm đối
  chứng. L40 vẫn chạy FP32 với batch size 2 theo config đã chốt.
...
- Static audit không thể bảo đảm mọi lỗi phần cứng/driver. Preflight trên đúng
  instance L40 là cổng xác nhận runtime cuối cùng.
