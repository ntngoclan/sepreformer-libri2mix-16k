...
class PIT_SDRi:
    ...
    def __call__(self, **kwargs):
        estims = torch.stack(kwargs['estims'], dim=0).squeeze(1).to(self.device)
        input_sizes = kwargs["input_sizes"].to(self.device)
        targets = [t.to(self.device) for t in kwargs["target_attr"]]
        targets = torch.stack(targets, dim=0).squeeze(1).to(self.device)
        input = torch.cat([kwargs['mixture'], kwargs['mixture']], dim=0).to(self.device)
        
        # Using fast-bss-eval directly on GPU (no need to transfer to CPU numpy)
        try:
            import fast_bss_eval
            sdr_out, _, _, _ = fast_bss_eval.bss_eval_sources(targets, estims)
            sdr_in, _, _, _ = fast_bss_eval.bss_eval_sources(targets, input)
            
            # fast_bss_eval returns SDR for each source in the batch. 
            # We move it to cpu numpy to maintain compatibility with the rest of the code.
            min_perutt_out = sdr_out.cpu().data.numpy()
            min_perutt_in = sdr_in.cpu().data.numpy()
            
        except ImportError:
            # Fallback to mir_eval if fast_bss_eval is not installed
            from mir_eval.separation import bss_eval_sources
