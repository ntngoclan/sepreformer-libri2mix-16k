...
class Model(torch.nn.Module):
    def __init__(self, 
        ...
        if module_sglr is not None and module_sglr.get('enabled', True):
            module_sglr = dict(module_sglr)
            module_sglr.pop('enabled', None)
            self.sglr = LTRR(**module_sglr)
        else:
            self.sglr = torch.nn.Identity()
    ...
    def forward(self, x):
        ...
        last_stage_output = self.sglr(last_stage_output)
