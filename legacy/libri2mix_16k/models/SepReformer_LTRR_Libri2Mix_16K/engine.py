...
def format_spk_list(x):
    """Ensure output is a list of tensors per speaker [Batch, Time]"""
    if isinstance(x, torch.Tensor) and x.dim() == 3:
        return [x[:, i, :] for i in range(x.shape[1])]
    if isinstance(x, list) and len(x) == 1 and isinstance(x[0], torch.Tensor) and x[0].dim() == 3:
        return [x[0][:, i, :] for i in range(x[0].shape[1])]
    return x
...
class Engine(object):
    def __init__(self, args, config, model, dataloaders, criterions, optimizers, schedulers, gpuid, device):
        ...
        self.pretrain_weights_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sep_output", "pretrain_weights")
        os.makedirs(self.pretrain_weights_path, exist_ok=True)
        self.scratch_weights_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sep_output", "scratch_weights")
        os.makedirs(self.scratch_weights_path, exist_ok=True)
        
        has_pretrain = any(
            file.endswith((".pth", ".pt", ".pkl"))
            for file in os.listdir(self.pretrain_weights_path)
        )

        self.checkpoint_path = self.pretrain_weights_path if has_pretrain else self.scratch_weights_path

        if getattr(args, "checkpoint", None):
            self.start_epoch = util_engine.load_checkpoint_file(
                args.checkpoint,
                self.model,
                self.main_optimizer,
                location=self.device
            )
            logger.info(f"Using explicit checkpoint: {os.path.expanduser(args.checkpoint)}")
        else:
            ...
            logger.info(f"Using latest checkpoint from: {self.checkpoint_path}")
    ...
    def _train(self, dataloader, epoch):
        ...
        for input_sizes, mixture, src, _ in dataloader:
            ...
            src = format_spk_list(src)
            estim_src = format_spk_list(estim_src)
            if isinstance(estim_src_bn, list):
                estim_src_bn = [format_spk_list(e) for e in estim_src_bn]
            else:
                estim_src_bn = [format_spk_list(estim_src_bn)]
    ...
    def _test(self, dataloader, wav_dir=None):
        ...
        with torch.inference_mode():
            csv_file_name_sisnr = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sep_output', 'test_SISNRi_value.csv')
            csv_file_name_sdr = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sep_output', 'test_SDRi_value.csv')
            with open(csv_file_name_sisnr, 'w', newline='') as csvfile_sisnr, open(csv_file_name_sdr, 'w', newline='') as csvfile_sdr:
                ...
                for input_sizes, mixture, src, key in dataloader:
                    ...
                    # Ensure matching lengths due to Conv1d stride dropping remainder samples
                    min_len = min(estim_src[0].shape[-1], src[0].shape[-1], mixture.shape[-1])
                    for i in range(len(estim_src)):
                        estim_src[i] = estim_src[i][..., :min_len]
                    ...
                    if self.engine_mode == "test_save":
                        ...
                        sample_rate = self.config["dataset"]["sampling_rate"]
                        mixture = np.nan_to_num(mixture)
                        mixture_peak = np.max(np.abs(mixture))
                        if mixture_peak == 0: mixture_peak = 1.0
                        
                        try:
                            sf.write(
                                os.path.join(wav_dir, key[0][:-4] + str(idx) + '_mixture.wav'),
                                0.5 * mixture / mixture_peak,
                                sample_rate
                            )

                            for i in range(self.config['model']['num_spks']):
                                src_audio = torch.squeeze(estim_src[i]).cpu().data.numpy()
                                src_audio = np.nan_to_num(src_audio)
                                src_peak = np.max(np.abs(src_audio))
                                if src_peak == 0: src_peak = 1.0
                                sf.write(
                                    os.path.join(wav_dir, key[0][:-4] + str(idx) + '_out_' + str(i) + '.wav'),
                                    0.5 * src_audio / src_peak,
                                    sample_rate
                                )
                        except Exception as e:
                            logger.warning(f"Failed to save audio {key[0]} at idx {idx}: {e}")
                    idx += 1
    ...
    def _inference_sample(self, sample):
        ...
        with torch.inference_mode():
            ...
            try:
                sf.write(sample[:-4]+'_in.wav', 0.9*mixture/mixture_peak, self.fs)
                for i in range(self.config['model']['num_spks']):
                    src = torch.squeeze(estim_src[i][...,:mixture.shape[-1]]).cpu().data.numpy()
                    src = np.nan_to_num(src)
                    src_peak = np.max(np.abs(src))
                    if src_peak == 0: src_peak = 1.0
                    sf.write(sample[:-4]+'_out_'+str(i)+'.wav', 0.9*src/src_peak, self.fs)
            except Exception as e:
                logger.warning(f"Failed to save audio for inference sample {sample}: {e}")

    
    @logger_wraps()
    def run(self):
        with torch.cuda.device(self.device):
            writer_src = SummaryWriter(os.path.join(os.path.dirname(os.path.abspath(__file__)), "sep_output/tensorboard"))
